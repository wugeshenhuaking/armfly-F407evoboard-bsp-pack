 ...
 *   diff 很小（高位为0）→ 未过期，diff 就是剩余毫秒
 *   diff 很大（bit31 =1）→ 已过期（无符号减法回绕到高位）
 *
 * 最大可定时时长: 2^31 ms ≈ 24.8 天（比原版 12.4 天翻倍）
 */

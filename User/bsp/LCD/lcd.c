...
#include "./BSP/LCD/framebuffer.h"   /* 软件显存镜像(WebUSB用) */
...
void lcd_draw_point(uint16_t x, uint16_t y, uint32_t color)
{
    ...
    LCD->LCD_RAM = color;
    fb_put_pixel(x, y, (uint16_t)color);   /* 镜像到外部SRAM显存(WebUSB) */
}
...
void lcd_init(void)
{
    ...
    LCD_FSMC_BTRX |= 15 << 0;   /* 地址建立时间(ADDSET)为15个HCLK 1/168M = 6ns * 15 = 90ns */

    /* 因为液晶驱动IC的读数据的时候，速度不能太快,尤其是个别奇葩芯片 */
    LCD_FSMC_BTRX |= 60 << 8;   /* 数据保存时间(DATAST)为60个HCLK = 6ns * 60 = 360ns */

    /* 写时序控制寄存器 */
    LCD_FSMC_BWTRX |= 0 << 28;  /* 模式A */
    LCD_FSMC_BWTRX |= 9 << 0;   /* 地址建立时间(ADDSET)为9个HCLK = 6ns * 9 = 54ns */

    /* 某些液晶驱动IC的写信号脉宽，最少也得50ns。 */
    LCD_FSMC_BWTRX |= 8 << 8;   /* 数据保存时间(DATAST)为8个HCLK = 6ns * 8 = 48ns */
...
void lcd_clear(uint16_t color)
{
    ...
    fb_clear(color);   /* 镜像整屏到外部SRAM显存(WebUSB) */
}
...
void lcd_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint32_t color)
{
    ...
    fb_fill_rect(sx, sy, ex, ey, (uint16_t)color);   /* 镜像到外部SRAM显存(WebUSB) */
}
